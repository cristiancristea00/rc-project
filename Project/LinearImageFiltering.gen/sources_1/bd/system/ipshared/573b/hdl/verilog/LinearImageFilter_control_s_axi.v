...
// ==============================================================
`timescale 1ns/1ps
module LinearImageFilter_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER                     = 7'h08,
    ADDR_ISR                     = 7'h0c,
    ADDR_IMAGE_OUT_OFFSET_DATA_0 = 7'h10,
    ADDR_IMAGE_OUT_OFFSET_CTRL   = 7'h14,
    ADDR_IMAGE_IN_OFFSET_DATA_0  = 7'h18,
    ADDR_IMAGE_IN_OFFSET_CTRL    = 7'h1c,
    ADDR_ROWS_DATA_0             = 7'h20,
    ADDR_ROWS_CTRL               = 7'h24,
    ADDR_COLS_DATA_0             = 7'h28,
    ...
    ADDR_STRIDE_COL_CTRL         = 7'h4c,
    ADDR_PADDING_DATA_0          = 7'h50,
    ADDR_PADDING_CTRL            = 7'h54,
    WRIDLE                       = 2'd0,
    WRDATA                       = 2'd1,
    WRRESP                       = 2'd2,
    WRRESET                      = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IMAGE_OUT_OFFSET_DATA_0: begin
                    rdata <= int_image_out_offset[31:0];
                end
                ADDR_IMAGE_IN_OFFSET_DATA_0: begin
                    rdata <= int_image_in_offset[31:0];
                end
                ...
                ADDR_STRIDE_COL_DATA_0: begin
                    rdata <= int_stride_col[31:0];
                end
...
assign image_out_offset  = int_image_out_offset;
assign image_in_offset   = int_image_in_offset;
...
// int_image_out_offset[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_image_out_offset[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMAGE_OUT_OFFSET_DATA_0)
            int_image_out_offset[31:0] <= (WDATA[31:0] & wmask) | (int_image_out_offset[31:0] & ~wmask);
    end
end

// int_image_in_offset[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_image_in_offset[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMAGE_IN_OFFSET_DATA_0)
            int_image_in_offset[31:0] <= (WDATA[31:0] & wmask) | (int_image_in_offset[31:0] & ~wmask);
    end
end
...
// int_kernel_offset[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_kernel_offset[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_KERNEL_OFFSET_DATA_0)
            int_kernel_offset[31:0] <= (WDATA[31:0] & wmask) | (int_kernel_offset[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_stride_row[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_STRIDE_ROW_DATA_0)
            int_stride_row[31:0] <= (WDATA[31:0] & wmask) | (int_stride_row[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_stride_col[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_STRIDE_COL_DATA_0)
            int_stride_col[31:0] <= (WDATA[31:0] & wmask) | (int_stride_col[31:0] & ~wmask);
    end
end
