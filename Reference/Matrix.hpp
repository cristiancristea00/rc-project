...
public:
    ...
    [[nodiscard]] constexpr auto rows() const -> std::size_t
    {
        return imageRows;
    }

    [[nodiscard]] constexpr auto cols() const -> std::size_t
    {
        return imageCols;
    }

    constexpr auto operator[](std::size_t const row, std::size_t const col) -> float &
    {
        return data[row * imageCols + col];
    }

    constexpr auto operator[](std::size_t const row, std::size_t const col) const -> float
    {
        return data[row * imageCols + col];
    }
